...
## 🌐 Система провайдеров данных и последние фичи

Проект построен не на одном жёстко зашитом источнике данных, а на модели провайдеров. `ProfileLoaderService` хранит набор реализаций `IProfileDataProvider` и выбирает первый доступный провайдер по приоритету. В текущем варианте используются:

- `PlayerPrefsProfileProvider` — быстрый локальный кэш для браузерной сессии.
- `FileProfileProvider` — файловое сохранение для локальной среды и редактора.
- `BackendProfileProvider` — внешняя API-интеграция с бэкендом.
- `GoogleSheetsProfileProvider` — синхронизация профиля и прогресса через Google Sheets.

То есть Google Sheets — это один из провайдеров данных, а не единственный источник истины. Игровой слой не зависит от конкретной реализации: он работает с общим интерфейсом профиля, а провайдеры могут быть локальными, серверными или табличными.

### Автозагрузка контента из Google Sheets

Контент в приложении подгружается по схеме “remote-first + cache-first”. `ContentLoaderService` автоматически скачивает четыре CSV-наборa из опубликованных Google Sheets:
...
Также есть отдельный лист конфигурации (`GoogleSheetsUrls.Config`), который хранит:

- `LAST MODIFY`;
- `APP URL`;
- `API KEY`.

Этот конфиг нужен, чтобы приложение знало, какой Google Apps Script и какая внешняя таблица сейчас активны, а также следило за изменениями контента. Если дата из `LAST MODIFY` отличается от сохранённой в кеше, приложение сбрасывает старые данные и перезагружает актуальные CSV-файлы. После этого данные сохраняются в `Application.persistentDataPath/ContentCache/` и используются при следующих запусках, что особенно важно для WebGL.

### Конфиги с другого листа и runtime-переконфигурация

Конфигурация не зашита в коде статически. `ContentLoaderService.CheckConfigSheet()` парсит CSV-конфиг и сохраняет его в объект `SheetsConfig`, а затем `RegistrationService.UpdateGoogleSheetConfig(_sheetConfig)` подмешивает активный URL и ключи в клиент Google Sheets.

Это даёт возможность перевести приложение на другой лист, другой Apps Script или другой набор таблиц без полной перекомпиляции и без переезда кода. Фактически конфиг-лист выступает как внешняя панель управления системой: он задаёт источник данных и адрес, куда приложение должно обращаться.

### Регистрация пользователя и обновление прогресса

В проекте есть отдельный поток регистрации пользователя. `RegistrationService.RegisterUserAsync(...)` формирует запись с полями:
...
После регистрации приложение сохраняет идентификатор пользователя и группу в `PlayerPrefs`, а затем при завершении уровня вызывает `SaveLevelResultAsync(...)` для обновления прогресса в Google Sheets. Это позволяет синхронизировать локальный прогресс игрока с удалённой таблицей, при этом сохраняя офлайн-фоллбэк при отсутствии сети.

### Как это выглядит в архитектуре

Рабочий поток следующий:

1. `ContentLoaderService` читает CSV из Google Sheets и сохраняет кэш.
2. `ProfileLoaderService` регистрирует провайдеры и выбирает активный источник данных.
3. `RegistrationService` работает с Google Sheets как с провайдером регистрации и обновления результатов.
4. `UserProfileService` и `ProgressService` используют загруженный профиль независимо от конкретного провайдера.

Таким образом, Google Sheets здесь — один из поддерживаемых провайдеров данных, а вся система построена так, чтобы легко добавлять новый источник, не ломая основной игровой цикл.
